...
import { ImagePlus, X, CheckCircle, Upload, Sparkles, AlertCircle } from 'lucide-react';
...
export default function MissionPage() {
  ...
  const missionId = /^\d+$/.test(normalizedMissionId) ? Number(normalizedMissionId) : undefined;
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          <div className="relative">
            <input
              ...
              onFocus={e => { if (!missionError) { e.target.style.borderColor = '#C9A84C'; e.target.style.boxShadow = '0 0 0 3px rgba(201,168,76,0.12)'; } }}
              onBlur={e => { if (!missionError) { e.target.style.borderColor = '#DFC98A'; e.target.style.boxShadow = '0 2px 8px rgba(201,168,76,0.08)'; } }}
            />
          </div>

          {/* Esito ricerca missione */}
          <AnimatePresence mode="wait">
            {missionError && (
              <motion.div
                key="error"
                initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="flex items-center gap-2 mt-2 text-sm" style={{ color: '#b45309' }}>
                <AlertCircle size={14} />
