...
var require_mime_types = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	function lookup(path) {
		...
		var extension = extname$1("x." + path).toLowerCase().slice(1);
...
*/
//#endregion
//#region src/server/api/health/GET.ts
...
//#endregion
//#region src/server/seo-host.ts
var SYSTEM_HOST_SUFFIXES = [
	".airoapp.ai",
	".test-airoapp.ai",
	".dev-airoapp.ai",
	".dev-godaddy.com"
...
app.use(import_express.urlencoded({ extended: true }));
...
app.get("/sitemap.xml", (req, res) => {
	if (isSystemHost(req)) {
		res.type("application/xml").set("Cache-Control", "public, max-age=60, must-revalidate").set("Vary", "Host").send(`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"/>\n`);
		return;
	}
	const base = baseUrl(req);
	const body = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${seoRoutes.filter((r) => typeof r.path === "string" && r.path.startsWith("/")).map((r) => {
...
});
{
	...
	app.use(import_express.static(clientDir, {
		index: false,
	...
	const loadTimeout = setTimeout(() => {
		...
		console.error("ssr.module.load-timeout", { timeoutMs: SSR_MODULE_LOAD_TIMEOUT_MS });
		process.exit(1);
	}, SSR_MODULE_LOAD_TIMEOUT_MS);
	loadTimeout.unref();
	import("./bin/entry-server-DBmMgn0o.js").then((mod) => {
		clearTimeout(loadTimeout);
		renderFn = mod.render;
	}, (err) => {
		clearTimeout(loadTimeout);
		console.error("ssr.module.load-failed", { error: err instanceof Error ? err.stack : String(err) });
		process.exit(1);
	});
	app.get(/.*/, async (req, res, next) => {
		...
		try {
			...
			const out = template.replace("<!--app-head-->", () => seoHead + result.head).replace("<!--app-html-->", () => result.html);
	...
	const shutdown = async (signal) => {
		...
		try {
			...
			if (error?.code !== "ERR_MODULE_NOT_FOUND") console.error("ssr.shutdown.db-import-failed", { error: error instanceof Error ? error.message : String(error) });
		}
		if (mod && typeof mod.closeConnection === "function") try {
			await mod.closeConnection();
			console.log("Database connections closed");
		} catch (error) {
			console.error("ssr.shutdown.db-close-failed", { error: error instanceof Error ? error.message : String(error) });
		}
		process.exit(0);
	};
...
//#endregion
export { __toCommonJS as a, app as default, __require as i, __esmMin as n, __toESM as o, __exportAll as r, __commonJSMin as t };
