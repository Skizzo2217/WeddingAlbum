...
import { C as require_react, _ as useIsomorphicLayoutEffect, g as PresenceContext, h as isHTMLElement, m as MotionConfigContext, p as usePresence, v as useConstant, x as require_jsx_runtime, y as LayoutGroupContext } from "./WeddingDecorations-Bix8aEST.js";
...
var SUPABASE_URL = "https://wdongpblggyyqgjeynbx.supabase.co";
var SUPABASE_PUBLISHABLE_KEY = "sb_publishable_huBVnOr4TziP7QVS6_3z4Q_kqOO6ASK";
var PHOTO_BUCKET = "photos";
var supabase = createClient(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, { auth: {
	persistSession: false,
	autoRefreshToken: false
} });
function slugify(value) {
	return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-+|-+$/g, "").toLowerCase().slice(0, 48);
}
function uniquePhotoPath(originalName) {
	...
	try {
		...
		const blob = await new Promise((resolve, reject) => {
			canvas.toBlob((result) => {
				...
				else reject(/* @__PURE__ */ new Error("Compressione foto non riuscita"));
			}, "image/jpeg", quality);
		});
...
async function uploadWeddingPhoto(input) {
	const compressed = await compressImage(input.file);
	const path = uniquePhotoPath(input.originalName ?? (compressed instanceof File ? compressed.name : void 0));
	const uploaderName = input.uploaderName?.trim() || void 0;
...
async function listWeddingPhotos() {
	const primaryResult = await supabase.from("photos").select("id,image_url,uploader_name,created_at,approved,mission_id").order("created_at", { ascending: false });
	let data = primaryResult.data;
	let error = primaryResult.error;
	if (error && error.message.includes("mission_id")) {
		const fallbackResult = await supabase.from("photos").select("id,image_url,uploader_name,created_at,approved").order("created_at", { ascending: false });
		data = fallbackResult.data;
		error = fallbackResult.error;
	}
	if (error) throw error;
	return (data ?? []).filter((row) => row.approved !== false && Boolean(row.image_url)).map((row) => {
		return {
			id: String(row.id),
			path: row.image_url,
			url: row.image_url,
			thumbnail_url: row.image_url,
			uploader_name: row.uploader_name || void 0,
			created_at: row.created_at ?? (/* @__PURE__ */ new Date()).toISOString(),
			is_photobooth: false,
			mission_id: row.mission_id ?? void 0
		};
	});
}
...
//#endregion
export { uploadWeddingPhoto as n, AnimatePresence as r, listWeddingPhotos as t };
