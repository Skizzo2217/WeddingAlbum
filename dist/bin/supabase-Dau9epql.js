...
import { S as require_react, _ as useConstant, b as require_jsx_runtime, f as usePresence, g as useIsomorphicLayoutEffect, h as PresenceContext, m as isHTMLElement, p as MotionConfigContext, v as LayoutGroupContext } from "./WeddingDecorations-CglGYRWY.js";
...
async function listWeddingPhotos() {
	...
	return (await Promise.all(folders.map(async (folder) => {
		const { data: nested, error: nestedError } = await supabase.storage.from(PHOTO_BUCKET).list(folder.name, {
			limit: 1e3,
			sortBy: {
				column: "created_at",
				order: "desc"
	...
	}))).flat().filter((item) => item.name.toLowerCase().match(/\.(jpe?g|png|webp|heic|heif)$/)).map((item) => {
		...
		return {
			...
			url,
			thumbnail_url: url,
			uploader_name: metadata?.uploader_name || void 0,
