...
import { C as require_react, _ as useIsomorphicLayoutEffect, g as PresenceContext, h as isHTMLElement, m as MotionConfigContext, p as usePresence, v as useConstant, x as require_jsx_runtime, y as LayoutGroupContext } from "./WeddingDecorations-BrcIrpFP.js";
...
async function compressImage(file, options = {}) {
	...
	try {
		...
		const blob = await new Promise((resolve, reject) => {
			canvas.toBlob((result) => result ? resolve(result) : reject(/* @__PURE__ */ new Error("Compressione foto non riuscita")), "image/jpeg", quality);
		});
...
async function uploadWeddingPhoto(input) {
	const [photo, thumbnail] = await Promise.all([compressImage(input.file, {
		maxWidth: 1600,
		quality: .82
	...
	const headers = input.uploadSession ? { "x-upload-session": input.uploadSession } : void 0;
	return responseJson(await fetch(apiUrl("/api/photos"), {
		method: "POST",
		body: formData,
		headers
	}));
...
async function listWeddingPhotos() {
	return (await responseJson(await fetch(apiUrl("/api/photos")))).photos;
}
//#endregion
